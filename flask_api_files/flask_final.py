...
def getPorts():
    ...
    # List country name and number of encounters.
    query = [{'$group': {'_id': "$CTY_NAME", 'count': {'$sum': 1}}}]
    results = list(import_ports.aggregate(query))
...
def getCommodity():
    ...
    # List items imported and encounters
    query = [{'$group': {'_id': "$I_COMMODITY_LDESC", 'count': {'$sum': 1}}}]
    results = list(import_commodities.aggregate(query))
